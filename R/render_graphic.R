...
panama <- ne_countries(scale = "large", returnclass = "sf") |> 
  filter(sovereignt == "Panama") |> 
...
longs <- map_df(c(-82, -80, -78), function(i) {
  tibble(x = i,
    ...
    st_buffer(125) |> 
    rename(geom = geometry) |> 
    transmute(population = 1)
})

lats <- map_df(c(7.5, 9.5), function(i) {
  tibble(y = i,
         x = seq(from = -83.5, to = -77, by = .5)) |> 
    st_as_sf(coords = c("x", "y"), crs = 4326) |> 
...
render_camera(phi = 35, zoom = .8, theta = -30)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(275, 275, 265, 265)),
